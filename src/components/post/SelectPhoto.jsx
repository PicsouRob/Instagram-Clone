...
function SelectPhoto({ postRef, setSelectPhoto }) {
    const handlePostRef = async () => {
        await postRef.current.click();
    ...
    return (
        <div
            class="fixed top-0 left-0 right-0 bottom-0 bg-[rgba(0,0,0,0.6)] z-40 w-full grid place-items-center px-6 cancle"
            onClick={cancleEvent}
        >
            ...
            <motion.div
                class="bg-white max-w-screen-lg rounded h-auto shadow w-full sm:w-[350px]"
                initial={{ y: "-100vh" }}
                animate={{ y: 0 }}
            >
                <div class="text-center p-3 border-b border-[#818181] font-bold text-[#333333]">
                    Create a new post
                </div>
                <div class="px-12 h-[380px] grid place-items-center text-center text-[20px]">
                    <div class="space-y-6 grid place-items-center">
                        ...
                        <button
                            class="px-3 py-2 bg-blue-medium text-white text-sm font-bold rounded-md"
...
SelectPhoto.propTypes = {
    postRef: PropTypes.func.isRequired,
    setSelectPhoto: PropTypes.func.isRequired,
}
